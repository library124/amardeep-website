...
class WorkshopAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'status_badge', 'participants_info', 'is_featured', 'edit_link', 'view_on_site'
    ]
    list_display_links = ['title']
    list_filter = [
    ...
    def edit_link(self, obj):
        return format_html(
            '<a href="/admin/portfolio_app/workshop/{}/change/" class="button" style="background: #007bff; color: white; padding: 5px 10px; border-radius: 4px; text-decoration: none;">✏️ Edit</a>',
            obj.id
        )
    edit_link.short_description = 'Actions'
    ...
    def view_on_site(self, obj):
        if obj.is_active:
            return format_html(
                '<a href="/workshops/{}" target="_blank" class="button" style="background: #28a745; color: white; padding: 5px 10px; border-radius: 4px; text-decoration: none;">👁️ View</a>',
                obj.slug
            )
    ...
    actions = [
        ...
        'mark_as_upcoming', 'mark_as_completed', 'duplicate_workshops'
    ]
    ...
    def duplicate_workshops(self, request, queryset):
        for workshop in queryset:
            workshop.pk = None
            workshop.title = f"Copy of {workshop.title}"
            workshop.slug = f"copy-of-{workshop.slug}"
            workshop.status = 'upcoming'
            workshop.registered_count = 0
            workshop.save()
        self.message_user(request, f'{queryset.count()} workshops duplicated.', messages.SUCCESS)
    duplicate_workshops.short_description = "📋 Duplicate workshops"
